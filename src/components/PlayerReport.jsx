...
const TABS = ['OVERVIEW', 'PASSING & VISION', 'DEFENSIVE', 'HEATMAPS']
...
function KPI({ label, value, sub }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 8, letterSpacing: 1.5, textTransform: 'uppercase', marginTop: 5, fontWeight: 700, fontFamily: 'var(--font)', opacity: 0.7 }}>{label}</div>
      {sub != null && (
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {sub != null && <span style={{ fontSize: 9, opacity: 0.45, fontFamily: 'var(--font)' }}>{sub}</span>}
...
function PercentBar({ label, made, total, color = '#0277B6' }) {
  ...
  return (
    <div style={{ padding: '6px 10px', borderBottom: B, fontFamily: 'var(--font)' }}>
      ...
      <div style={{ height: 6, background: '#eee', border: '1px solid #000' }}>
    ...
    <div ref={ref} style={{ background: '#fff', fontFamily: 'var(--font)', maxWidth: compact ? '100%' : 920, margin: '0 auto', padding: compact ? 12 : 20 }}>
      ...
      <div style={{ border: BT, marginBottom: 12, display: 'flex' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 36, fontWeight: 700, lineHeight: 1, fontFamily: 'var(--font)' }}>{jerseyNumber}</div>
          <div style={{ fontSize: 8, letterSpacing: 1, marginTop: 4, opacity: 0.85, textTransform: 'uppercase', fontFamily: 'var(--font)' }}>NO.</div>
        </div>
        {/* Name / position */}
        <div style={{ flex: 1, padding: '10px 14px' }}>
          ...
          <div style={{ fontSize: 10, marginTop: 5, display: 'flex', gap: 14 }}>
        ...
        {matchInfo?.home_team_score != null && (
          <div style={{
            borderLeft: BT, padding: '10px 16px', display: 'flex',
            flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minWidth: 80,
          }}>
            <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: 3, fontFamily: 'var(--font)' }}>
              {matchInfo.home_team_score}–{matchInfo.away_team_score}
            </div>
      ...
      {/* ── KPI Hero Stats ────────────────────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 0, marginBottom: 12, border: BT }}>
        {[
          ...
          { label: 'INTERCEPTS', value: stats.interceptions ?? 0,                    sub: null },
          { label: 'DRIBBLES',   value: stats.dribbles ?? 0,                         sub: `${stats.dribbleRate ?? 0}% SUCC` },
        ].map(({ label, value, sub }, i) => (
          <div key={label} style={{ borderRight: i < 5 ? B : 'none' }}>
      ...
      {/* ── Tab Nav ───────────────────────────────────────────── */}
      {!compact && (
        <div style={{ display: 'flex', borderBottom: BT, marginBottom: 0 }}>
          {TABS.map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
      ...
      <div style={{ border: BT, borderTop: 'none' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: compact ? '1fr' : '1fr 1fr', gap: 0 }}>
            ...
            <div>
              <Module title="SHOOTING">
                ...
                {stats.totalShots > 0 && (
                  <StatRow label="Conversion Rate" value={`${Math.round((stats.goals / stats.totalShots) * 100)}%`} />
                )}
              </Module>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
            ...
            <div>
              {hasPassEvents
                ...
                : <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 300, border: B, opacity: 0.3, fontFamily: 'var(--font)', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase' }}>NO PASS DATA</div>
              }
            </div>
          </div>
        )}
        ...
        {/* COMPACT (compare mode): show heatmap inline */}
        {compact && (
          <Module title="TOUCH HEATMAP">
            <HeatMap events={stats.allEvents ?? []} teamColor={accentColor} />
          </Module>
